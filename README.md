...
- 阶段 F：MIG-001 至 MIG-014 在当前会话得到预期行为；敏感文件与常见密钥特征检查通过。新会话动态加载、Skill 隐式触发、真实 UI 及工程发布候选仍未验证，因此尚不能宣称完整 Harness 迁移完成。

## 阶段 F 证据边界

- 已实际验证：C3 Memory 路由、GitHub 仓库读取、缺失 Skill 报告、禁止文件检查和常见密钥特征检查。
- 已按规则验证：L0-L3 授权、事实与推断区分、冲突处理、Skill 候选阈值和拒绝授权后的保持现状。
- 明确未验证：全新会话独立加载、Skill 隐式触发、真实网页截图 Audit、PR/CI、回滚、日志、监控和发布后冒烟测试。
